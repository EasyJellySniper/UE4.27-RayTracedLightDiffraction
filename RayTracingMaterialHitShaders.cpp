	...
	const typename FUniformLightMapPolicy::ElementDataType& RESTRICT LightMapElementData)
{
	...
	// SQUALL MOD - add custom hit group define
	TMeshProcessorShaders<
		FMeshMaterialShader,
		FMeshMaterialShader,
		FMeshMaterialShader,
		FMeshMaterialShader,
		FMeshMaterialShader,
		FCustomMaterialCHS> CustomRayTracingShaders;
	// END MOD
	...
	// SQUALL MOD - Set custom CHS for light diffraction material
	if (MaterialResource.IsUsedWithLightDiffraction())
	{
		CustomRayTracingShaders.RayHitGroupShader = MaterialResource.GetShader<FCustomMaterialCHS>(VertexFactory->GetType());
	}
	// END MOD
	...
	// SQUALL MOD - decide hit group for use
	if (MaterialResource.IsUsedWithLightDiffraction())
	{
