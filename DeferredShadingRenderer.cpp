...
/* SQUALL MOD: include custom stuff */
#include "RayTracing/RayTracingCustom.h"
#include "Renderer/Private/Interfaces/WaveVolumeInterface.h"
IWaveVolumeInterface* IWaveVolumeInterface::Interface = nullptr;
/* END MOD */
...
bool FDeferredShadingSceneRenderer::DispatchRayTracingWorldUpdates(FRHICommandListImmediate& RHICmdList)
{
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
	{
		...
		/* SQUALL MOD - prepare custom ray tracing */
		PrepareRayTracingCustoms(View, RayGenShaders);
		/* END MOD */
...
void FDeferredShadingSceneRenderer::Render(FRHICommandListImmediate& RHICmdList)
{
	...
	/* SQUALL MOD - Add custom ray tracing stuff and custom depth pass after z prepass */
#if RHI_RAYTRACING
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		RenderRayTracingCustomDepth(GraphBuilder, GetSceneTextureParameters(GraphBuilder, SceneTextures), Views[ViewIndex]);
	}
#endif
	/* END MOD */
	...
	/* SQUALL MOD - Add ray tracing light diffraction before any other opaque effects */
#if RHI_RAYTRACING
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		RenderRayTracingLightDiffraction(GraphBuilder, GetSceneTextureParameters(GraphBuilder, SceneTextures), Views[ViewIndex]);
	}
#endif
	/* END MOD */
	...
	/* SQUALL MOD: Add wave volume pass after fogs */
	if (IWaveVolumeInterface* WaveVolumeInterface = IWaveVolumeInterface::Get())
	{
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
		{
			FViewInfo& View = Views[ViewIndex]; 
			WaveVolumeInterface->WaveVolumeRendering(GraphBuilder, View, GFastVRamConfig);
		}
	}
	/* END MOD */
...
bool AnyRayTracingPassEnabled(const FScene* Scene, const FViewInfo& View)
{
	if (ShouldRenderRayTracingAmbientOcclusion(View)
		...
		/* SQUALL MOD: Check if should render custom ray tracing */
		|| ShouldRenderRayTracingCustom()
		/* END MOD */
